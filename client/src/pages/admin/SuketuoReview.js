...
const SuketuoReview = () => {
  ...
  const [suketuoApplications, setSuketuoApplications] = useState([]);
  ...
  const fetchApplications = async () => {
    setLoading(true);
    try {
      const response = await axios.get('/api/credits/admin/suketuo');
      setSuketuoApplications(response.data);
    } catch (error) {
      console.error('获取素拓学分申请失败:', error);
      message.error('获取素拓学分申请失败');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApprove = async () => {
    try {
      ...
      await axios.post(`/api/credits/admin/suketuo/${currentApplication.id}/approve`, {
  ...
  const handleReject = async () => {
    try {
      ...
      await axios.post(`/api/credits/admin/suketuo/${currentApplication.id}/reject`, {
        feedback: values.feedback
      });
              ...
              Modal.info({
                ...
                content: (
                  <div>
                    ...
                    <p><strong>活动类型:</strong> {record.activityType}</p>
                    <p><strong>活动级别:</strong> {record.level}</p>
                    <p><strong>活动日期:</strong> {moment(record.activityDate).format('YYYY-MM-DD')}</p>
      ...
      {currentApplication && (
        <>
          <div style={{ marginBottom: 24 }}>
            ...
            <p><strong>活动名称:</strong> {currentApplication.activityName}</p>
            <p><strong>活动类型:</strong> {currentApplication.activityType}</p>
            <p><strong>活动级别:</strong> {currentApplication.level}</p>
  ...
  return (
    <div className="suketuo-review-container">
      {renderReviewHeader()}
      
      <Card>
        <Table
          columns={columns}
          dataSource={suketuoApplications}
          rowKey="id"
          loading={loading}
